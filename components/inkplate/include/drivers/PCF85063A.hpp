...
class PCF85063A {
	public:
		...
		void setDateTime(struct tm *now);
		void readDateTime(struct tm *now);
		void readTime();
		void enableAlarm();	// called on setAlarm()
		void setAlarm(int8_t alarm_second, int8_t alarm_minute, int8_t alarm_hour, int8_t alarm_day, int8_t alarm_weekday);
		void readAlarm();
